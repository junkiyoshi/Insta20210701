...
void ofApp::setup() {
	...
	for (int x = 0; x < this->cap_size.width; x += this->rect_size) {
		...
		for (int y = 0; y < this->cap_size.height; y += this->rect_size) {
			...
			this->gap_list.push_back(this->rect_size);
		}
	}
...
void ofApp::update() {
	...
	int frame_index = (int)((ofGetFrameNum() + 5) * 0.42) % this->number_of_frames;
	int next_frame_index = (frame_index + 1) % this->number_of_frames;

	for (int i = 0; i < this->gap_list.size(); i++) {

		this->gap_list[i] = this->gap_list[i] <= 0 ? 0 : this->gap_list[i] - 0.02;
	}
...
void ofApp::draw() {
	...
	for (int i = 0; i < this->rect_frame_list.size(); i++) {
		...
		auto power = this->gap_list[i] > 0.5 ? 0 : ofMap(this->gap_list[i], 0, 0.5, 1, 0);

		auto tremor = glm::vec3(
			ofMap(ofNoise(ofRandom(1000), ofGetFrameNum() * 0.005), 0, 1, this->rect_size * -6.5, this->rect_size * 6.5) * power,
			ofMap(ofNoise(ofRandom(1000), ofGetFrameNum() * 0.005), 0, 1, this->rect_size * -6.5, this->rect_size * 6.5) * power,
			ofMap(ofNoise(ofRandom(1000), ofGetFrameNum() * 0.005), 0, 1, this->rect_size * -6.5, this->rect_size * 6.5) * power);
